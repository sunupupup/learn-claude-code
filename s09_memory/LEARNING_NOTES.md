...
>
> 当前状态：s09 基础机制已掌握，可以开始 s10；生产级 Memory 治理暂存于 W-2026-013，尚未启动。
...
详细学习任务见 W-2026-013：Memory 生产实践与生命周期治理。本章目前先记录入口和关键意识。
...
需要说明谁读取索引、谁读取正文、两者进入请求的位置，以及 tab 记忆是否一定会被加载。

我的回答：

- 在 loop 开始前读取 MEMORY.md，放入 SYSTEM prompt；
- 根据“请创建 test.py”找需要加载的 Memory 文件；
- 索引进入 SYSTEM，正文进入最新的 user message；
- tab 记忆不一定加载，要看描述是否与当前需求相关。

🔴 **已验证理解**：索引进入 SYSTEM，选中的正文进入当前 user message；Memory 是否被选择取决于相关性判断。

校准：

- `build_system()` 读取索引；`read_memory_file()` 读取正文；
- `select_relevant_memories()` 通常参考最近最多 3 条 user message，不只参考当前一句话；
- 选择结果是 filename 列表，`load_memories()` 再读取这些文件的正文；
- 当前代码把正文放在原始用户问题的前面，即 prepend，而不是 append 到问题后面；
- 索引即使正文没有被选中，仍然可以出现在 SYSTEM 中；
- side-query 失败、空结果、fallback 未命中或压缩后定位失效时，正文也可能没有注入。

结论：第 1 题通过，已经掌握 Memory 的选择范围和注入位置。

### 10.2 consolidation 写入过程中崩溃
...
- 旧记忆文件全部被删除；
- 新文件没有创建成功，并且无法回滚；
- 可以先把旧文件移动到 temp，等新文件创建成功后再删除 temp；
- 如果进程崩溃，可以从 temp 恢复到主目录。

🔴 **已验证理解**：直接 `unlink()` 后发生崩溃，旧集合可能无法恢复，系统也没有可靠回滚能力；把旧数据暂存起来比直接删除更安全。

校准：

- 当前代码删除的是旧的单条 Memory 文件，`MEMORY.md` 会被保留，但可能留下指向旧文件的失效链接；
- 新文件可能一个都没有，也可能只写入一部分；
- “移动到 temp 再恢复”属于备份/隔离思路，但逐文件移动仍可能在中途崩溃；
- 更完整的保护是：写入唯一的 staging 目录，完整生成并校验新文件和 MEMORY.md 后，再原子发布；
- 另一个必要保护是文件锁或单写者机制，并配合事务 ID、状态标记和重启恢复；
- LLM 返回空数组或非法结构时，不应先删除旧集合。

结论：第 2 题核心判断和恢复方向正确；需要补充“原子发布”和“并发控制”。该问题已加入 W-2026-013 作为重点生产实践任务。
...
- 启动 W-2026-013，系统学习 Memory 的生产治理。

### 是否可以开始 s10

可以开始 s10。理由是 s09 的基础机制和代码主链路已经理解，剩余内容属于生产级扩展，不阻塞下一章。W-2026-013 保持 ready，以后可以单独启动，不需要把它混入 s10 的基础学习。
