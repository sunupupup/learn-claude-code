...
已按用户“详细调研”的要求完成启动前资料调查，见 [自进化 Agent 原理与实现方案调研](../../learning-notes/work-pool/W-2026-026-study-agent-self-evolution-harnesses-自进化Agent/RESEARCH_REPORT.md)。报告是本次证据、源码发现、版本差异与学习建议的主要来源。

覆盖 Reflexion、Voyager、ACE、GEPA、Hermes、DGM，以及近期 Self-Harness、HarnessEvolve、HarnessDev 和 JIT-Agent；对照 SEAL、Agent Lightning 与本卡原有项目。后续默认按报告的入门顺序学习，原有双项目深读保留为选修。

本次是调研资料准备，不视为正式互动学习已启动或完成；状态保持 `ready`。下方 2026-08-28 的 Freshness Notes 是历史记录，当前核验范围及限制以新报告为准。新增报告尚未经过用户复述或实验验收，不替代将来的 `LEARNING_NOTES.md`。

## 本次学习入口：先理解原理与实现方案（2026-09-19）

### Objective

用户希望新增“最近很火的自进化 Agent”学习内容，主要了解原理和实现方案。本主题已有任务卡，因此复用 W-2026-026，不重复建档。当前仍为 `ready`，仅补充学习范围，尚未正式启动。

默认先走 **D1–D2 理论入门线**：能解释机制、比较方案和读懂最小设计即可；原有 D4 源码、实验和模型适配内容保留为进阶选修。此处不是最新生态调研，下面已有项目与来源属于历史研究线索，本轮未重新核验，不据此指定当前最佳方案。

### Work：按问题逐节学习

1. **到底什么在变？** 区分一次回答内的反思重试、跨任务保存经验、修改提示词或技能、修改执行程序、更新模型参数；先回答“不训练模型能不能改善后续表现”。
2. **改进闭环如何形成？** 围绕“执行任务 → 收集反馈 → 分析失败 → 提出候选变化 → 独立评测 → 接受或拒绝 → 保留版本与回滚”逐步建立模型；明确谁提出修改、谁验证、谁决定启用。
3. **有哪些实现路线？** 比较经验记忆、提示词/技能优化、工作流/工具代码修改、模型后训练四条路线；逐项说明变化对象、持久化位置、触发时机、反馈来源、成本和适用条件。
4. **最小系统怎样组织？** 以“Agent 从失败任务中提出一条新规则”为贯穿案例，画清任务执行器、经验记录、候选生成器、评测器和版本管理之间的数据流。先读懂设计，不要求立即写代码或选择框架。
5. **怎样证明真的变好了？** 比较固定基线与候选版本，区分用于改进的样例和未参与改进的留出测试；讨论随机波动、只记住测试答案、模型自评偏差，以及质量提升是否只是增加了推理成本。
6. **哪些变化应被拒绝？** 分析错误经验固化、记忆污染、越权修改、旧能力退化和无法回滚；用一个“新规则改善一类任务却破坏另一类”的反例检验方案。

### 术语入口

| 术语 | 中文含义与系统层次 | 本阶段需分清的边界 |
| --- | --- | --- |
| Self-evolution | 自进化，本任务中的系统改进研究主题 | 不预设为模型自动更新参数，也不把一次重试当成跨任务学习 |
| Harness | Agent 外围执行与控制程序，属于应用执行层 | 承担循环、工具、状态和约束，与模型权重分开 |
| Memory / Skill | 记忆 / 可复用技能，属于应用经验与策略层 | 保存经历与保存可执行做法不同，两者都需验证质量 |
| Evaluation / Eval | 评测，属于验证层 | 用任务和判定标准检查效果，不等于 Agent 自称进步 |
| Post-training | 后训练，属于模型训练层 | 改变模型参数，与运行时装配经验或提示词不同 |

### Expected Output 与 Success Criteria

- 一张“改变什么、存在哪里、如何验证”的实现路线对照表。
- 一张最小改进闭环图，能指出候选版本进入实际使用之前的检查位置。
- 能用自己的话解释“自我反思、长期记忆、自修改、模型训练”的区别。
- 能分析一个候选被接受和一个被拒绝的纸面案例，说明证据与局限；纸面推演不记为运行验证。
- 学习启动后的结论记录在 `learning-notes/work-pool/W-2026-026-study-agent-self-evolution-harnesses-自进化Agent/LEARNING_NOTES.md`，本轮不提前生成最终笔记。

### Boundaries 与 Start Trigger

- 用户明确说“开始 W-2026-026”或“开始自进化 Agent 学习”后，依照 [启动提示词](../../PROMPT_4_WORK_POOL_LEARN.md) 和 [Spec 规范](../README.md) 流转。
- 入门只需先理解模型调用、工具执行和跨任务状态；缺少这些概念时先补一个小例子，不要求先完成全部 D4 前置章节。
- 下文原有 Start Trigger、Preferred Order、Expected Output 和 Acceptance Criteria 对应**进阶完整研究**；完成入门不表示完整研究已完成，剩余范围继续明确记录。
- 本阶段不要求下载项目、调用付费模型、训练权重或运行自修改代码。进入具体库、API、安装或配置学习时，再使用 Context7 并核对官方资料与固定版本。

### Related

- [Memory 学习任务](../changes/C-2026-005-study-memory-production-practices.md)：复用经验写入、召回与纠错基础。
- [完成验证学习任务](../changes/C-2026-003-study-task-completion-verification.md)：关联“执行完成”和“候选改进有效”的证据区别。
- [后训练与严格 JSON 学习任务](../changes/C-2026-006-study-strict-json-output-post-training.md)：需要深入参数更新时再延伸。
